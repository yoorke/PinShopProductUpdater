...
namespace PinShopProductUpdater
{
    public class EweBL
    {
        ...
        public string[] ParseProductsForSaving(string eweCategory, string[] eweSubcategories, int categoryID, string logFilename, int eweCategoryID)
        {
            ...
            for (int i = 0; i < eweSubcategories.Length; i++)
            {
                ...
                if (xmlDoc != null)
                {
                    ...
                    foreach (XmlNode xmlNode in nodeList)
                    {
                        ...
                        product.PriceRebate = xmlNode.SelectSingleNode("price_rebate").InnerText.Replace('.',',').Trim();
                        ...
                        product.Images = xmlNode.SelectSingleNode("images") != null ? xmlNode.SelectSingleNode("images").OuterXml.Trim() : string.Empty;
                        ...
                        product.EweCategoryID = eweCategoryID.ToString();
            ...
            return products.Count > 0 ? sendEweProducts(products, eweCategory, categoryID, logFilename, eweSubcategories) : new string[] { "0", "0" };
            //return 0;
        }
        ...
        private XmlDocument getXml(string category, string subcategory, bool images, bool attributes)
        {
            ...
            try
            {
                ...
                string url = ConfigurationManager.AppSettings["eweConnectionString"] + "&secretcode=" + ConfigurationManager.AppSettings["secretCode"];
                if (images)
                    url += "&images=1";
                if (attributes)
                    url += "&attributes=1";
